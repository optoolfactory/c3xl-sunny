...
class Controls:
  def __init__(self, CI=None):
    ...
    self.captured_startup_memory_probe = False
  ...
  def update_events(self, CS):
    ...
    if LOW_MEMORY_STARTUP_PROBE and not SIMULATION and not self.captured_startup_memory_probe:
      if self.sm.seen['deviceState'] and self.sm.seen['procLog'] and len(self.low_memory_proc_history) > 0:
        startup_report = self._build_low_memory_report()
        startup_report["trigger"]["reason"] = "controlsd_startup_probe"
        sentry.capture_memory_usage(startup_report)
        self.captured_startup_memory_probe = True
